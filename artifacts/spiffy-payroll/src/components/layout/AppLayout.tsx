...
import { BookOpen, CalendarDays, Home, BarChart3, DollarSign, Settings, LogOut, Menu, X } from 'lucide-react';
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  return (
    <div className="flex h-screen w-full bg-[#F9FAFB] overflow-hidden">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-[220px] bg-[#0D1B4E] h-full flex-shrink-0">
        <div className="p-6">
          <img src={logoPath} alt="Spiffy Cleaning" className="h-12 w-auto mb-8" />
          <nav className="space-y-2">
            {navItems.map(item => {
              const isActive = location === item.path || location.startsWith(`${item.path}/`);
              return (
                <Link key={item.path} href={item.path}>
                  <div className={`flex items-center space-x-3 px-4 py-3 rounded-r-md cursor-pointer transition-colors ${
                    isActive 
                      ? 'border-l-4 border-[#1DC8FF] text-[#1DC8FF] bg-white/5 font-medium' 
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="lg:hidden bg-[#0D1B4E] text-white h-16 flex items-center justify-between px-4 z-10">
          <img src={logoPath} alt="Spiffy" className="h-8 w-auto" />
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-[280px] bg-[#0D1B4E] h-full flex flex-col shadow-2xl animate-in slide-in-from-left">
            <div className="p-4 flex items-center justify-between border-b border-white/10">
              <img src={logoPath} alt="Spiffy Cleaning" className="h-8 w-auto" />
