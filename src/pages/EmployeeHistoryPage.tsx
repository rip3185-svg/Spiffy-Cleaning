...
export default function EmployeeHistoryPage() {
  ...
  return (
    <div className="pb-24">
      ...
      <p className="text-white/50 text-sm mb-6">{t.history.subtitle}</p>
      ...
      {summaries.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-[#1A1A2A] mb-2">{t.history.noHistory}</h3>
          <p className="text-gray-500">{t.history.noHistorySub}</p>
        </div>
      ) : (
              ...
              <div key={summary.weekStart} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 bg-gray-50 p-4 text-sm">
                    <p className="text-gray-600 mb-2">{t.history.summaryDetails}</p>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span className="text-gray-500">{t.history.jobsPay}</span>
                        <span className="font-medium">${summary.totalPay.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">{t.history.drivePay}</span>
                        <span className="font-medium">${summary.drivePay.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">{t.history.deductions}</span>
